...
void controller::computeTorques()
{
    ...
    taux = Ixx*(lambda1*errorqxd + q*r*(Izz-Iyy)/Ixx + k1*signSx);
    tauy = Iyy*(lambda2*errorqyd + p*r*(Ixx-Izz)/Iyy + k2*signSy);
    tauz = Izz*(lambda3*errorqzd + p*q*(Iyy-Ixx)/Izz + k3*signSz);
